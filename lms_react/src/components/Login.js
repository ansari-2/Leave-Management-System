...
const Login = () => {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setToken(data);
        localStorage.setItem('authToken', data.username);//for storing username
        localStorage.setItem('token',data.token) // for storing token
        history("/dashboard");
  ...
  return (
    <div>
      {/* Navigation Bar */}
     <div className="nav-bar" style={{ display: "flex", justifyContent: "center", alignItems: "center"}}>
       
         <div className="navbar-title" style={{ textAlign: 'center', marginLeft: '20px' }}>Leave Management System</div>
